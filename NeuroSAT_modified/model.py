...
class NeuroSATNet(nn.Module):
	...
	def get_hidden(self, x, mask):
		...
		L_vote = self.L_vote(L).squeeze(2).reshape(bsz, -1, 2)
		L_prob = F.softmax(L_vote, dim=2)
		return L, L_prob
	
	def forward(self, x, mask):
		_, L_prob = self.get_hidden(x, mask)
		L_entropy = (-L_prob * torch.log(L_prob)).sum(dim=2)
		L_score = (L_entropy * mask).sum(dim=1)
		return L_score