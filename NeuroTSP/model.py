...
class NeuroTSPNet(nn.Module):

	def __init__(self, hidden_dim, iters):
		super(NeuroTSPNet, self).__init__()
		self.V_init = nn.Linear(1, hidden_dim)
		...
		self.E_vote = MLP(hidden_dim, hidden_dim, 1)
		self.iters = iters
	
	def forward(self, EV, W, C, mask):
		bsz, n_edge, n_vertex = EV.shape
		one = torch.ones(1, device=EV.device)
		V_init = self.V_init(one).unsqueeze(0).unsqueeze(0).repeat((bsz, n_vertex, 1))
		E_init = self.E_init(torch.cat((W, C), dim=2))
		...
		E_vote = self.E_vote(E).squeeze(2)
		E_mean = (E_vote * mask).sum(dim=1)
		return torch.sigmoid(E_mean)